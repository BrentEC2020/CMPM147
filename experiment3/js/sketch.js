...
function generateGrid(numCols, numRows) {
  ...
  // Place a cottage tile randomly in the center of one rectangle
  ...
  grid[chestX][chestY] = "C"; // Assume "C" represents a cottage tile
  grid[chestX - 2][chestY - 2] = "C";
  grid[chestX][chestY - 2] = "C";
  grid[chestX - 3][chestY] = "C";
  grid[chestX - 4][chestY - 4] = "C";
  ...
  for(let i = 0; i < grid.length; i++) {
    for(let j = 0; j < grid[i].length; j++) {
      if (gridCheck(grid, i, j, ".")) {
        ...
        placeTile(i, j, floor(random(9, 11)), floor(random(18, 19))); 
      } else if (gridCheck(grid, i, j, "C")) {
        placeTile(i, j, 26, floor(random(0, 3))); 
      } else {
        drawContext(grid, i, j, ".", floor(random(3)), floor(random(1)));
      }
    }
  }
}
